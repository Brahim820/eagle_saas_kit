...
from eagle.addons.auth_signup.models.res_partner import random_token as generate_token
...
class SaasContract(models.Model):
    ...
    user_cost = fields.Float(string="User Price")
    ...
    on_create_email_template = fields.Many2one('mail.template', default=lambda self: self.env.ref('eagle_saas_kit.client_credentials_template'), string="Client Creation Email Template")
    ...
    def send_expiry_mail(self):
        for obj in self:
            template = self.env.ref('eagle_saas_kit.contract_expiry_template')
    ...
    def send_subdomain_email(self):
        for obj in self:
            ...
            template = self.env.ref('eagle_saas_kit.client_subdomain_template')
            mail_id = template.send_mail(obj.id)
            current_mail = self.env['mail.mail'].browse(mail_id)
            current_mail.send()
                    ...
                    if obj.saas_client and obj.saas_client.database_name:
                        ...
                        if response:
                            ...
                            if user_count > obj.previous_cycle_user:
                                ...
                                # raise Warning("%r"%len(arrer_response))
