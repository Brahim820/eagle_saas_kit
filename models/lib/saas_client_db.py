...
@check_error
def create_new(client,database_name,user,passwd,admin_passwd): #send Eagle admin password. Using default (Admin) for now
...
def create_saas_client(operation = None, eagle_url=None, eagle_username = None, eagle_password = None, base_db=None, database_name=None,modules_list=[],admin_passwd = "admin"):
    ...
    client = connect_db(eagle_url,base_db, eagle_username, eagle_password, flag = (True if operation != "clone" else False)) # base db for cloningelse it doesn't matter
    if not client:
        return response
...
#     
    if operation == 'clone':
        ...
        response['db_create'] = create_new(client, database_name, eagle_username, eagle_password,admin_passwd) # new database name along with credentials

    if len(modules_list) and operation == 'install':
       client = connect_db(eagle_url,database_name, eagle_username, eagle_password, flag = False)
       if not client:
